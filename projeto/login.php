...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT pass FROM usuarios WHERE email = '$email' AND firstname = '$firstname' ";
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($pass, $pass_hash)) {
            ...
            $sql = "SELECT id,firstname,lastname,email,birthdate,gender FROM usuarios WHERE email = '$email' AND firstname = '$firstname'";
            $result = $conn->query($sql);
            ...
            header("Location: erro copy.html");
            exit();
        }
    } else {
